...
public class PersonDao {
    ...
    public boolean update(Person person) {
        ...
        try {
            ...
            Connection con = cp.getConnection();
            if (con != null) {
                String sql = "update person set name=?,fname=?,age=?,gender=?,hobbies=?,image=? where id=?";
                PreparedStatement smt = con.prepareStatement(sql);
                smt.setString(1, person.getName());
                smt.setString(2, person.getFname());
                smt.setInt(3, person.getAge());
                smt.setString(4, person.getGender());
                smt.setString(5, person.getHobbies());
                smt.setString(6, person.getImage());
                smt.setInt(7, person.getId());
                int n = smt.executeUpdate();
  ...
  /*//for checking functions  that it is running properly or not...
